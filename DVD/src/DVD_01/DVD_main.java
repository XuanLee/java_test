 ...
 public static void add(String name[]){
	...
	for(int i=0;i<name.length;i++){
				if(name[i] !=null){
					...
					System.out.println((i+1)+"\t"+name[i]+"\t\t"+sta+"\t\t"+Sdate+"\t\t"+count[i]+"次");
	}
	}
}
	//删除
...
//		
	...
	if(state[index]==0){//如果可借
		...
		while(true){
			...
			if(riqi<date[index]){
				System.out.println("归还日期应该比借出日期大,请重新输入");
				riqi=in.nextInt();
				continue;
			}
			break;
	...
	//给name数组增加一个数
	public static void addData(String name[],String str,Scanner in){		
		int count=1;
		for(int i=0;i<name.length;i++){
			if(str.equals(name[i])){
				System.out.println("DVD名相同");
				while(str.equals(name[i])){//dvd名相同重新输入，输入五次退出方法
					count++;
					if(count>5){
						System.out.println("输入超过5次退出");
						return;
					}
					System.out.println("请重新输入DVD名");
					str=in.nextLine();
				}
			}
		}
